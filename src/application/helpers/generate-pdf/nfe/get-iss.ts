...
export function getIss({
  ...
  if (total.ISSQNtot?.vServ !== undefined ||
    ...
    campo({ value: total.ISSQNtot?.vServ ?? '', x: 137.5, y: 771, largura: 132.5, alinhamento: 'right', ajusteX, ajusteY, doc, margemEsquerda, margemTopo });
    
    titulo({ value: 'BASE DE CÁLCULO DO ISSQN', x: 273.5, y: 763, largura: 132.5, ajusteX, ajusteY, doc, margemEsquerda, margemTopo });
    campo({ value: total.ISSQNtot?.vBC ?? '', x: 273.5, y: 771, largura: 132.5, alinhamento: 'right', ajusteX, ajusteY, doc, margemEsquerda, margemTopo });
    
    titulo({ value: 'VALOR TOTAL DO ISSQN', x: 409.5, y: 763, largura: 175.5, ajusteX, ajusteY, doc, margemEsquerda, margemTopo });
    campo({ value: total.ISSQNtot?.vISS ?? '', x: 409.5, y: 771, largura: 175.5, alinhamento: 'right', ajusteX, ajusteY, doc, margemEsquerda, margemTopo });
  }
