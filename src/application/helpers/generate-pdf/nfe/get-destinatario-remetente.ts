...
export function getDestinatarioRemetente({
    ...
    campo({ value: dest.CPF?.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4') ?? '', x: 358, y: y + 18, largura: 133.5, ajusteX, ajusteY, doc, margemEsquerda, margemTopo });
    campo({ value: dest.CNPJ?.replace(/(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, '$1.$2.$3/$4-$5') ?? '', x: 358, y: y + 18, largura: 133.5, ajusteX, ajusteY, doc, margemEsquerda, margemTopo });

    titulo({ value: 'DATA DA EMISSÃO', x: 495, y: y + 11, largura: 90, ajusteX, ajusteY, doc, margemEsquerda, margemTopo });
    campo({ value: format(parseISO(ide.dhEmi), 'dd/MM/yyyy HH:mm:ss'), x: 495, y: y + 18, largura: 90, ajusteX, ajusteY, doc, margemEsquerda, margemTopo });

    titulo({ value: 'ENDEREÇO', x: 1.5, y: y + 30, largura: 272, ajusteX, ajusteY, doc, margemEsquerda, margemTopo });
    campo({ value: dest.enderDest?.xLgr && dest.enderDest?.nro ? `${dest.enderDest.xLgr}, ${dest.enderDest.nro}` : dest.enderDest?.xLgr ?? '', x: 1.5, y: y + 38, largura: 272, alinhamento: 'left', tamanho: DEFAULT_NFE.tamanhoDaFonteDoCampo - 0.5, ajusteX, ajusteY, doc, margemEsquerda, margemTopo });
    ...
    campo({ value: ide.dhSaiEnt !== undefined ? format(parseISO(ide.dhSaiEnt), 'dd/MM/yyyy HH:mm:ss') : '', x: 495, y: y + 38, largura: 90, ajusteX, ajusteY, doc, margemEsquerda, margemTopo });

    titulo({ value: 'MUNICÍPIO', x: 1.5, y: y + 51, largura: 272, ajusteX, ajusteY, doc, margemEsquerda, margemTopo });
    campo({ value: dest.enderDest?.xMun ?? '', x: 1.5, y: y + 58, largura: 272, alinhamento: 'left', ajusteX, ajusteY, doc, margemEsquerda, margemTopo });
    ...
    campo({ value: dest?.IE?.replace(/(\d{3})(\d{3})(\d{3})(\d{3})/, '$1.$2.$3.$4') ?? '', x: 398, y: y + 58, largura: 93, ajusteX, ajusteY, doc, margemEsquerda, margemTopo });
