  ...
  if (cobr !== undefined && Object.keys(cobr).length > 0) {
    ...
    if (cobr.fat !== undefined) {
      doc.font('negrito')
        ...
        .font('negrito').text(' Valor Original:', { continued: true })
        .font('normal').text(` ${Intl.NumberFormat('pt-BR', { minimumFractionDigits: 2 }).format(Number(cobr.fat.vOrig))}`, { continued: true })
        .font('negrito').text(' Valor Desconto:', { continued: true })
        .font('normal').text(` ${Intl.NumberFormat('pt-BR', { minimumFractionDigits: 2 }).format(Number(cobr.fat.vDesc))}`, { continued: true })
        .font('negrito').text(' Valor Líquido:', { continued: true })
        .font('normal').text(` ${Intl.NumberFormat('pt-BR', { minimumFractionDigits: 2 }).format(Number(cobr.fat.vLiq))}.`, { continued: true });
    }

    if (cobr.dup !== undefined) {
      cobr.dup.forEach((dup, index) => {
        doc.font('negrito').text(' Duplicata:', { continued: true })
          .font('normal').text(` ${dup.nDup}`, { continued: true });
        doc.font('negrito').text(' Valor:', { continued: true })
          .font('normal').text(` ${Intl.NumberFormat('pt-BR', { minimumFractionDigits: 2 }).format(Number(dup.vDup))}`, { continued: true });
        doc.font('negrito').text(' Vencimento:', { continued: true })
          .font('normal').text(` ${(format(parseISO(dup.dVenc), 'dd/MM/yyyy'))}`, { continued: true });
