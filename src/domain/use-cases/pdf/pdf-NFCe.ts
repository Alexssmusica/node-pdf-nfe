...
async function pdfNFCe(nf:NFeProc , pathLogo: string): Promise<PDFKit.PDFDocument> {
    ...
    const doc = new pdfkit({
      ...
      margins: { top: margemPadrao, bottom: 0, left: margemPadrao, right: margemPadrao },
    });
    ...
    normal({ doc, value: `${emit.enderEmit.xLgr}, <span class="math-inline">\{emit\.enderEmit\.nro\}</span>{emit.enderEmit.xCpl !== undefined ? ' ' + emit.enderEmit.xCpl : ''}. ${emit.enderEmit.xBairro}, <span class="math-inline">\{emit\.enderEmit\.xMun\}\-</span>{emit.enderEmit.UF}. ${emit.enderEmit.CEP}`, x: 0, y: doc.y, largura: larguraPagina, tamanho: 8, ajusteX: 0, ajusteY: 0, margemEsquerda: 0, margemTopo: 0 });
    doc.y += 5;
   ...
   negrito({ doc, value: 'Consulte pela chave de acesso em', x: 0, y: doc.y, largura: larguraPagina - margemPadrao * 2, tamanho: 8, ajusteX: 0, ajusteY: 0, margemEsquerda: 0, margemTopo: 0, alinhamento: 'center' });
    ...
    normal({ doc, value: `Data Autorização: ${format(parseISO(infProt.dhRecbto), 'dd/MM/yyyy HH:mm:ss')}` ?? '', x: doc.x, y: doc.y, largura: 112, tamanho: 7, ajusteX: 0, ajusteY: 0, margemEsquerda: 0, margemTopo: 0, alinhamento: 'left' });
    doc.y += 7;
