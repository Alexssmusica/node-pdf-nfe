...
async function pdfNFe(nf, pathLogo) {
    ...
    for (let i = paginas.start; i < paginas.start + paginas.count; i++) {
        doc.switchToPage(i);
        (0, italico_1.italico)({ doc, value: `FOLHA ${i + 1}/${paginas.start + paginas.count}`, x: 241.2, y: i === 0 ? 141.2 : 97.4, largura: 98.5, alinhamento: 'center', tamanho: 8, ajusteX, ajusteY, margemEsquerda, margemTopo });
    }
    doc.flushPages();
    doc.end();
    return doc;
