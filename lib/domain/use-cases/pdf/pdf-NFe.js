...
const cria_layout_1 = require("../../../application/helpers/generate-pdf/nfe/cria-layout");
...
async function pdfNFe(nf, pathLogo) {
    ...
    (0, gerar_itens_1.gerarItens)({ ajusteX, ajusteY, nf, doc, larguraDoFormulario, margemDireita, margemEsquerda, margemTopo, pathLogo, folha: 0 });
    const paginas = doc.bufferedPageRange();
    for (let i = paginas.start; i < paginas.start + paginas.count; i++) {
        doc.switchToPage(i);
        await (0, cria_layout_1.criaLayout)({ ajusteX, ajusteY, nf, doc, larguraDoFormulario, margemDireita, margemEsquerda, margemTopo, pathLogo, folha: i });
        (0, italico_1.italico)({ doc, value: `FOLHA ${i + 1}/${paginas.start + paginas.count}`, x: 241.2, y: 141.2, largura: 98.5, alinhamento: 'center', tamanho: 8, ajusteX, ajusteY, margemEsquerda, margemTopo });
    }
    doc.flushPages();
    doc.end();
    return doc;
